...
def mostra_dados(nome, telefone, aniversario, email):
    print(f"Nome: {nome} | Telefone: {telefone} | Aniversário: {aniversario} | E-mail: {email}")
...
def altera():
    ...
    p = pesquisa(nome)
    if p != None:
        nome_antigo, telefone, aniversario, email = agenda[p]
        print("Encontrado:")
        mostra_dados(nome_antigo, telefone, aniversario, email)
