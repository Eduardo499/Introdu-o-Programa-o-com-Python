...
def grava():
    global agenda, agenda_alterada
    if agenda_alterada:  # Verifica se a agenda foi alterada
        try:
            ...
            for e in agenda:
                ...
                arquivo.write("%s#%s\n" % (nome, telefone))
            arquivo.close()
            salva_nome_arquivo(nome_arquivo)  # Salva o nome do arquivo
            agenda_alterada = False  # Reinicia a variável após gravar
            print("Agenda gravada com sucesso.")
        except (IOError, OSError) as e:
            print(f"Erro ao gravar a agenda: {e}")
    else:
...
def lê():
    global agenda, agenda_alterada
    try:
        nome_arquivo = le_nome_arquivo()  # Lê o nome do último arquivo
        if nome_arquivo:
            ...
            for l in arquivo.readlines():
                ...
                agenda.append([nome, telefone])
            arquivo.close()
            agenda_alterada = False  # Reinicia a variável após carregar os dados
